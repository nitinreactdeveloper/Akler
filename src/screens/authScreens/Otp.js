...
const Otp = ({ navigation, route }) => {
    ...
    return (
        <BaseScreen title={''} navigation={navigation} renderChild={Content({ navigation, route })} notGoBack={true} />
    )
}
...
const Content = ({ navigation, route }) => {
    ...
    const handleSubmit = () => {
        ...
        if (isValid) {
            // verifyOtp(mobile,otp1+otp2+otp3+otp4)
    ...
    // useEffect(() => {
    //     console.log(mobile, 'params data')
    //     ref1.current.focus()
    // }, [])
    ...
    return (
        <ScrollView style={styles.contentScroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps={'always'}>
            ...
            <View style={{ display: 'flex', justifyContent: 'center', alignSelf: 'center', marginVertical: 20, marginBottom: 30 }}>
                ...
                <Text style={{
                    fontSize: 16, color: '#857C7C', alignSelf: 'center', textAlign: 'center',
            ...
            <TouchableOpacity style={[styles.solidBtn, styles.whiteBtn]}
                onPress={() => { alert('Suceess', setotp1(null), setotp2(null), setotp3(null), setotp4(null)) }}>
                <Text style={[styles.solidBtnTxt, { color: '#539BEF', fontFamily: "Poppins-Bold", width: '100%', textAlign: 'right' }]}>Resend OTP</Text>
            </TouchableOpacity>
